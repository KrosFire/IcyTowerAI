class Neuron {
  ...
  constructor(no) {
    ...
    this.outputConnections = []; //new ArrayList<Synapse>();
    this.layer = 0;
  }
  ...
  engage() {
      ...
      for(var i = 0; i < this.outputConnections.length; i++) { //for each connection
        if(this.outputConnections[i].enabled) { // don't do shit if not enabled
  ...
  sigmoid(x) {
      return 1.0 / (1.0 + pow(Math.E, -1*x)); //todo check pow
    }
    //----------------------------------------------------------------------------------------------------------------------------------------------------------
